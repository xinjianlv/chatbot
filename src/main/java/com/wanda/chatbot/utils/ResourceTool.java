...
public class ResourceTool {
	 ...
	 * @return 文件内容
	 */
	public static List<String> readResourceFile(String filePackage , Charset charset){
		List<String> sb = new ArrayList<String>();
		BufferedReader fin = null;
		try{
			InputStream is = ResourceTool.class.getClassLoader().getResourceAsStream(filePackage);
			fin = new BufferedReader(new InputStreamReader(is, charset.name()));
			String line = null;
	...
	public static HashMap<String, Integer> LoadStrIntegerFromFile(String filePackage,
			int lineOffset, int keyIndex, int valIndex, String split,
			Charset charset) throws IOException {
		String line = null;
		...
		while ((line = sr.readLine()) != null) {
			...
			String strKey = tmp[keyIndex].trim();
			Integer strVal = Integer.parseInt(tmp[valIndex].trim());
			dict.put(strKey, strVal);
		}
		sr.close();
	 ...
	 * @return
	 */
	public static Set<String> readString(String filePackage , String splitPunc , int contentIndex , Charset charset){
		 try{
			 Set<String> set = new HashSet<String>();
			 ArrayList<String> lst = (ArrayList<String>) readResourceFile(filePackage  ,  charset);
			 for(String line : lst){
	...
	public static String readString(String filePath , String splitPunc , Charset charset){
		try{
			List<String> list = readResourceFile(filePath , charset);
	...
	public static void main(String[] args) {
		try{
			Set<String> set = readString("./data/cnCi.txt", "\t", 0 ,Charset.forName("utf-8"));
